...
// [LOCAL PATCH — OIRA] added optional per-element values array; see MPSICS.h.
u64 MPSICardSumParty(u32 idx, u32 numParties, u32 numElements,
                     std::vector<block> &set,
                     u32 numThreads,
                     const std::vector<u64>* values)
{
    // [LOCAL PATCH — OIRA] validate values shape if provided.
    if (values != nullptr && values->size() != numElements) {
        std::cout << "MPSICardSumParty [OIRA]: values.size() (" << values->size()
    ...
    // [LOCAL PATCH — OIRA] parallel value table (same shape as simHashTable).
    // Populated only for non-P_0 senders when the caller provided explicit
    // values. Empty otherwise.
    std::vector<std::vector<u64>> simHashValues(numBins);
    ...
    if(idx != 0)
    {
    	...
    	for (u64 i = 0; i < numBins; ++i)
    	{
    	    ...
    	    // [LOCAL PATCH — OIRA] resize the parallel values vector.
    	    if (values != nullptr) simHashValues[i].resize(size);
    	    for (u64 p = 0; p < size; ++p)
    	    {
    	    	...
    	    	// [LOCAL PATCH — OIRA] value for slot (i,p) = caller's values[b].
    	    	if (values != nullptr) simHashValues[i][p] = (*values)[b];
    	    }
    	}
    }
    ...
    if(idx == 0){
        ...
        // [LOCAL PATCH — OIRA] pass optional simHashValues if caller provided
        // values; nullptr otherwise (preserves upstream behavior).
        opprfSendPSICS64(numParties, numElements, cuckooSeed, simHashTable, chl[0], itemSum, opprfPath2, numThreads,
                         (values != nullptr) ? &simHashValues : nullptr);
