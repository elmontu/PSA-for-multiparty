...
// get the OPPRF key
// [LOCAL PATCH — OIRA] signature now takes an optional per-slot value array.
// See PMT.h for rationale.
void opprfSendPSICS64(u32 numParties, u32 numElements, block cuckooSeed,
                      ...
                      std::string volePath, u32 numThreads,
                      const std::vector<std::vector<u64>>* simHashValues)
{
    PRNG prng(sysRandomSeed());
    u32 numBins = simHashTable.size();

    // [LOCAL PATCH — OIRA] shape check for the values array if provided.
    if (simHashValues != nullptr) {
        if (simHashValues->size() != numBins) {
            std::cout << "opprfSendPSICS64 [OIRA]: simHashValues size ("
                      << simHashValues->size() << ") != numBins ("
                      << numBins << ")" << std::endl;
            return;
        }
        for (u32 i = 0; i < numBins; ++i) {
            if ((*simHashValues)[i].size() != simHashTable[i].size()) {
                std::cout << "opprfSendPSICS64 [OIRA]: bin " << i
                          << " values size mismatch" << std::endl;
                return;
            }
        }
    }
    ...
    for (u32 i = 0; i < numBins; ++i)
    {
    	...
    	{
    	    auto hyj = simHashTable[i][p];
            // [LOCAL PATCH — OIRA] the "value" (u64 to be summed for this slot)
            // is either the explicit values array (caller-provided) or, for
            // backward compatibility with the upstream test_mpsics probe, the
            // high 64 bits of the hash-table entry (which was set to the key).
            u64 slotValue = (simHashValues != nullptr)
                            ? (*simHashValues)[i][p]
                            : hyj.mData[1];
            u64 w = slotValue - mLabel[i];// w = x-s
