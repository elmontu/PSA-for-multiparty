...
- **Remaining runtime hang (genuinely deferred):** end-to-end `frontend -mpsa` runs hang somewhere in the early handshake phase. All 3N-1 sockets ESTAB cleanly (`netstat` confirms 8/8 connections for N=3) and bytes flow on the spSocks, but the processes never produce output. Suspect order-of-ops in MpSpHandshake/sessionId-broadcast/MPSI when multiple per-sender messages stack up on a single spSock. Need to bisect with `std::cerr` at each protocol step. Unit tests are unaffected since they exercise cryptographic correctness in isolation.
